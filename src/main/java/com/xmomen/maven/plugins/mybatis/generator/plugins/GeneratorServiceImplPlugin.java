 ...
 */
public class GeneratorServiceImplPlugin extends CommonPlugin {
    ...
    public List<GeneratedJavaFile> contextGenerateAdditionalJavaFiles(IntrospectedTable introspectedTable) {
        logger.info(MessageFormat.format("Generating ServiceImpl class for {0}", introspectedTable.getTableConfiguration().getTableName()));
        setProperty();
        validateProperty();
        if(modulePackage != null && targetPackage == null){
            targetPackage = modulePackage + ".service.impl";
        }
        TemplatePropertyDefine templatePropertyDefine = new TemplatePropertyDefine(
                "{0}ServiceImpl.java",
                targetPackage,
                FreemarkerDefine.SERVICE_IMPL_TEMPLATE);
