          ...
          <table class="table table-striped b-t b-light">
            ...
            <#if fieldList?exists>
              <#list fieldList as field>
                <#if !field.primaryKey>
                <td>
                  <a ng-bind="item.${field['javaProperty']}"></a>
                </td>
                </#if>
              </#list>
            </#if>
      ...
      <form class="form-horizontal" ug-validate="${domainObjectName}DetailForm" name="${domainObjectName}DetailFormName">
        ...
        <#if fieldList?exists>
          ...
          <div class="form-group">
              <label class="col-md-2 control-label">${field['fieldComment']}<span class="vaild">*</span></label>
              <div class="col-md-10">
                  <input class="form-control" type="text" name="${field['javaProperty']}" placeholder="请输入${field['fieldComment']}"
                         ng-disabled="pageSetting.formDisabled"
                         <#if !field.nullable>
                         required="true"
                         </#if>
                         data-rule-maxlength="${field.maxLength}"
                         ng-model="${domainObjectName}.${field['javaProperty']}">
              </div>
          </div>
          </#if>
          </#list>
